...
const deleteMutation = useMutation({
  mutationFn: async (templateId: number) => {
    return await reportTemplatesApi.delete(templateId);
  },
  onSuccess: () => {
  ...
  onError: (error: any) => {
    toast.error(error.response?.data?.detail || 'Failed to delete report template');
  },
});
...
const toggleMutation = useMutation({
  mutationFn: async (templateId: number) => {
    return await reportTemplatesApi.toggle(templateId);
  },
  onSuccess: (data) => {
    queryClient.invalidateQueries({ queryKey: ['report-templates'] });
